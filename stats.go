...
const daysInLastSixMonths = 180
const outOfRange = 999
const weeksInLastSixMonths = 26

// stats calculates and prints the stats.
func stats(email string, repos []string) {
    fmt.Printf("Scanning %d repositories for email: %s\n", len(repos), email)
...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
    ...
    ref, err := repo.Head()
    if err != nil {
        // Skip repositories without HEAD (empty repos, etc.)
        return commits
    }
    // get the commits history starting from HEAD
    iterator, err := repo.Log(&git.LogOptions{From: ref.Hash()})
    if err != nil {
        // Skip repositories with log errors
        return commits
    }
    // iterate the commits
    offset := calcOffset()
    err = iterator.ForEach(func(c *object.Commit) error {
        ...
        if daysAgo != outOfRange && daysAgo >= 0 && daysAgo <= daysInLastSixMonths {
            commits[daysAgo]++
        }
